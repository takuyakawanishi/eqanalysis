...
def add_trailing_hms_b(str_date_b):
    str_date_b = str_date_b + " 00:00:00"
    return str_date_b


def add_trailing_hms_e(str_date_e):
    str_date_e = str_date_e + " 23:59:59"
    return str_date_e
    ...
    dbc.Container([
        dbc.Row([
            dbc.Col([
                ...
                html.Div(id="stations-periods_to"),
                html.Br(),
...
@app.callback(
    ...
    Output('stations-periods_to', 'children'),
...
def show_available_periods(group_dict):
    ...
    dfsp = eqa.find_available_periods_ts(meta, station)
    stations_periods_dict = (pd.DataFrame(dfsp)).to_dict('records')
    dfsp_from = dfsp[["station", "from"]]
    stations_periods_show = html.Div([
        ...
        dbc.Container([
            dash_table.DataTable(
                ...
                columns=[{'name': i, 'id': i} for i in dfsp_from.columns]),
        ], style={"font-size": "1rem"})
    ])
    dfsp_to = dfsp[["station", "to"]]
    stations_periods_show_to = html.Div([
        dbc.Container([
            dash_table.DataTable(
                data=stations_periods_dict,
                columns=[{'name': i, 'id': i} for i in dfsp_to.columns]),
        ], style={"font-size": "1rem"})
    ])
    return stations_periods_dict, stations_periods_show, stations_periods_show_to
...
def setting_the_analysis_period(sy, sm, sd, ey, em, ed):
    ...
    analysis_from = add_trailing_hms_b(analysis_from)
    analysis_to = str(ey) + '-' + str(em).zfill(2) + '-' + str(ed).zfill(2)
    analysis_to = add_trailing_hms_e(analysis_to)
...
def draw_intensity_frequency(
    ...
    station = meta_1.at[0, "code_prime"]
    print(station)
    print("draw_intensity_frequency")
    print("set_1_dict = ", set_1_dict)
    frequency, fit, sum_0 = eqa.find_intensity_frequency_regression_summarize_ts(
